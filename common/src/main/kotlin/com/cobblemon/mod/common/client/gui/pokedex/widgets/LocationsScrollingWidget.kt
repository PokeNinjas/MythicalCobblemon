...
import com.cobblemon.mod.common.api.text.notBold
import com.cobblemon.mod.common.api.text.notStrikethrough
import com.cobblemon.mod.common.api.text.notUnderline
...
class LocationsScrollingWidget(val pX: Int, val pY: Int): ScrollingWidget<LocationsScrollingWidget.LocationWidgetEntry>(
    width = 131,
    height = 40,
    left = pX,
    top = pY + 10,
    ...
    var locations: List<ClientsidePokedexSpawnDetailInfo> = emptyList()

    override fun getX() = pX
    override fun getY() = pY

    var entriesAdded = false

    fun setEntries() {
        if (entriesAdded) return
        entriesAdded = true
        locations.forEach {
            addEntry(LocationWidgetEntry(this, it))
        }
    }
    ...
    override fun renderWidget(context: GuiGraphics, mouseX: Int, mouseY: Int, delta: Float) {
        drawScaledText(
            ...
            text = Component.literal("Locations").bold(),
            x = pX,
            y = pY - 10,
            shadow = true
        )
        ...
        if (locations.isEmpty()) {
            drawScaledText(
                context = context,
                text = Component.literal("No locations!"),
                x = pX + (width / 2) - 7,
                y = pY + (height / 2) - 3,
                shadow = false,
                colour = 0x606B6E,
    ...
    override fun getEntry(index: Int): LocationWidgetEntry {
        return children()[index] as LocationWidgetEntry
    }

    class LocationWidgetEntry(val parent: LocationsScrollingWidget, val location: ClientsidePokedexSpawnDetailInfo): Slot<LocationWidgetEntry>() {
        val species = location.pokemonProperties.species?.let { PokemonSpecies.getByName(it) }
        val renderablePokemon = species?.let { RenderablePokemon(it, location.pokemonProperties.aspects) }
        ...
        override fun render(
            ...
            if (mouseX in x..(x+131) && mouseY in y..(y+25)) {
                ...
                val component = Component.literal("Conditions").bold().underline().withColor(0xF7B27C)
                for (line in lines) {
                    if ("<st>" in line) {
                        component.append(Component.literal("\n" + line.replace("<st>", "")).notBold().notUnderline().strikethrough().withColor(0xFA6969))
                    } else if ("biome" in line.lowercase()) {
                        component.append(Component.literal("\n" + line).notBold().notUnderline().notStrikethrough().withColor(0x8DFCCE))
                    } else {
                        component.append(Component.literal("\n" + line).notBold().notUnderline().notStrikethrough().withColor(0xFCEB8D))
                    }
                }
